        ...
        for row in cur.fetchall():
            ...
            if table_info[schema_name].get(table_name) is None:
                table_info[schema_name][table_name] = {
                    'is_view': is_view, 'row_count': row_count, 'columns': {}}

            col_name = col_info[0]

            table_info[schema_name][table_name]['columns'][col_name] = Column(
                *col_info)
...
def write_sql_data_type_md(mdata, col_info):
    ...
    if col_info.sql_data_type == 'bit' and col_info.character_maximum_length > 1:
        mdata = metadata.write(mdata, ('properties', c_name), 'sql-datatype',
                               "bit({})".format(col_info.character_maximum_length))
    else:
        mdata = metadata.write(mdata, ('properties', c_name),
                               'sql-datatype', col_info.sql_data_type)
...
def discover_columns(connection, table_info):
    ...
    for schema_name in table_info.keys():
        for table_name in table_info[schema_name].keys():
            ...
            table_pks = [col_name for col_name,
                         col_info in columns.items() if col_info.is_primary_key]
            ...
            column_schemas = {col_name: schema_for_column(
                col_info) for col_name, col_info in columns.items()}
...
def attempt_connection_to_db(conn_config, dbname):
    ...
    try:
        ...
        LOGGER.warning(
            'Unable to connect to %s. This maybe harmless if you have not desire to replicate from this database: "%s"', dbname, err)
        return False
...
def do_discovery(conn_config):
    ...
    with post_db.open_connection(conn_config) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor, name='stitch_cursor') as cur:
            ...
            if conn_config.get('filter_dbs'):
                sql = post_db.filter_dbs_sql_clause(
                    sql, conn_config['filter_dbs'])

            LOGGER.info("Running DB discovery: %s with itersize %s",
                        sql, cur.itersize)
            cur.execute(sql)
            found_dbs = (row[0] for row in cur.fetchall())

    filter_dbs = filter(lambda dbname: attempt_connection_to_db(
        conn_config, dbname), found_dbs)
    ...
    if len(all_streams) == 0:
        raise RuntimeError(
            '0 tables were discovered across the entire cluster')
...
def do_sync_incremental(conn_config, stream, state, desired_columns, md_map, default_replication_key):
    replication_key = md_map.get((), {}).get(
        'replication-key') or default_replication_key
    ...
    illegal_bk_keys = set(stream_state.keys()).difference(set(
    ...
    state = incremental.sync_table(
        conn_config, stream, state, desired_columns, md_map, default_replication_key)
...
def sync_method_for_streams(streams, state, default_replication_method, default_replication_key):
    ...
    for stream in streams:
        ...
        replication_key = stream_metadata.get((), {}).get(
            'replication-key', default_replication_key)
        ...
        if replication_method not in set(['LOG_BASED', 'FULL_TABLE', 'INCREMENTAL']):
            raise Exception("Unrecognized replication_method {} for stream {}".format(
                replication_method, stream['tap_stream_id']))

        md_map = metadata.to_map(stream['metadata'])
        desired_columns = [c for c in stream['schema']['properties'].keys(
        ) if sync_common.should_sync_column(md_map, c)]
        desired_columns.sort()

        if len(desired_columns) == 0:
            LOGGER.warning(
                'There are no columns selected for stream %s, skipping it', stream['tap_stream_id'])
            continue

        if replication_method == 'LOG_BASED' and stream_metadata.get((), {}).get('is-view'):
            raise Exception('Logical Replication is NOT supported for views. Please change the replication method for {}'.format(
                stream['tap_stream_id']))
...
def sync_traditional_stream(conn_config, stream, state, sync_method, end_lsn, default_replication_key):
    ...
    if sync_method == 'full':
        ...
        state = do_sync_full_table(
            conn_config, stream, state, desired_columns, md_map)
    elif sync_method == 'incremental':
        state = singer.set_currently_syncing(state, stream['tap_stream_id'])
        state = do_sync_incremental(
            conn_config, stream, state, desired_columns, md_map, default_replication_key)
    elif sync_method == 'logical_initial':
        state = singer.set_currently_syncing(state, stream['tap_stream_id'])
        LOGGER.info("Performing initial full table sync")
        state = singer.write_bookmark(
            state, stream['tap_stream_id'], 'lsn', end_lsn)

        sync_common.send_schema_message(stream, [])
        state = full_table.sync_table(
            conn_config, stream, state, desired_columns, md_map)
        state = singer.write_bookmark(
            state, stream['tap_stream_id'], 'xmin', None)
...
def sync_logical_streams(conn_config, logical_streams, state, end_lsn):
    if logical_streams:
        LOGGER.info("Pure Logical Replication upto lsn %s for (%s)", end_lsn, list(
            map(lambda s: s['tap_stream_id'], logical_streams)))
        logical_streams = list(map(lambda s: logical_replication.add_automatic_properties(
            s, conn_config), logical_streams))

        state = logical_replication.sync_tables(
            conn_config, logical_streams, state, end_lsn)
...
def do_sync(conn_config, catalog, default_replication_method, default_replication_key, state):
    ...
    LOGGER.info("Selected streams: %s ", list(
        map(lambda s: s['tap_stream_id'], streams)))
    ...
    sync_method_lookup, traditional_streams, logical_streams = sync_method_for_streams(
        streams, state, default_replication_method, default_replication_key)

    if currently_syncing:
        LOGGER.info("found currently_syncing: %s", currently_syncing)
        currently_syncing_stream = list(
            filter(lambda s: s['tap_stream_id'] == currently_syncing, traditional_streams))
        if currently_syncing_stream is None:
            LOGGER.warning("unable to locate currently_syncing(%s) amongst selected traditional streams(%s). will ignore",
    ...
    for stream in traditional_streams:
        state = sync_traditional_stream(
            conn_config, stream, state, sync_method_lookup[stream['tap_stream_id']], end_lsn, default_replication_key)

    logical_streams.sort(key=lambda s: metadata.to_map(
        s['metadata']).get(()).get('database-name'))
    for dbname, streams in itertools.groupby(logical_streams, lambda s: metadata.to_map(s['metadata']).get(()).get('database-name')):
        ...
        state = sync_logical_streams(
            conn_config, list(streams), state, end_lsn)
    return state
...
def main_impl():
    ...
    try:
        if bool(args.config.get('use_ssh_tunnel')) == True:
            LOGGER.info(
                f"use_ssh_tunnel is set to true; connecting to {args.config['host']}:{args.config['port']} via {args.config['ssh_jump_server']}:{args.config['ssh_jump_server_port']}")
            tunnel = sshtunnel.open_tunnel(
                (args.config['ssh_jump_server'], int(
                    args.config['ssh_jump_server_port'])),
                ssh_username=args.config['ssh_username'],
                ssh_pkey=args.config['ssh_private_key_path'],
                ssh_private_key_password=args.config[
                    'ssh_private_key_password'] if 'ssh_private_key_password' in conn_config else None,
                remote_bind_address=(
                    args.config['host'], int(args.config['port'])),
                # local_bind_address=('0.0.0.0', 10022) # leaving this off uses a random local port
            )
            tunnel.start()
            time.sleep(1)
            # rewrite the config to go through our tunnel
        ...
        post_db.cursor_iter_size = int(args.config.get('itersize', '100000'))
        ...
        if args.discover:
            ...
            state = args.state
            default_replication_method = args.config.get(
                'default_replication_method')
            default_replication_key = args.config.get(
